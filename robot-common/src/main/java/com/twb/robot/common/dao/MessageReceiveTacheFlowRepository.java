...
public interface MessageReceiveTacheFlowRepository extends JpaRepository<MessageReceiveTacheFlow, Integer> {
	...
	public List<MessageReceiveTacheFlow> getMsgRecQueueListByTache(@Param("tache_code")String tache_code); 
	

	@Query(value= "select a.* from message_receive_tache_flow a where a.state='0' limit 0,1000",nativeQuery = true)
	public List<MessageReceiveTacheFlow> getMessageReceiveQueueList(); 
